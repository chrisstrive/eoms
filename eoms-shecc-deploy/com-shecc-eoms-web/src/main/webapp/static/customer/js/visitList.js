...
function initDataVisit(pageindx){
  ...
  $.ajax({
       ...
       url:cpath + "/visitRecord.htm?method=visitList&khId=" + $('#customerForm input[name=id]').val(),//目标地址
       ...
       success:function(json) {
                ...
                $.each(pageData, function(i, data) {
                    ...
                    trs += "<tr>" + 
                    		...
                    		"</td>" +
                    		"<td align='center'>" +
                    		"<a href=\"javascript:deleteVisitRecord('" + data.id + "')\">删除</a>&nbsp;" + 
                    		"</td>" +
                    		"</tr>";
                 tbody+=trs; 
                });
...
function deleteVisitRecord(id){
	$.ajax({
		type: "POST",//用POST方式传输
	    dataType:"json",//数据格式:JSON
	    url:cpath + "/visitRecord.htm?method=deleteVisitRecord&id=" + id,//目标地址
	    async:false,//作用是防止在ajax成功调用之前就调用$("#PaginationVisitRecord").Pagination,这个时候数据个数还没有初始化
	    beforeSend:function(){
        	// 隐藏错误提示DIV
        	//$("#showMsgDiv").hide();
