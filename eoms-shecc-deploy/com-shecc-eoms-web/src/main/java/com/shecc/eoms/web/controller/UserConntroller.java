...
public class UserConntroller extends MultiActionController {
	 ...
	 * @return
	 */
	public ModelAndView userDefault(HttpServletRequest request,
	 ...
	 * @return
	 */
	public ModelAndView addDispth(HttpServletRequest request,
			HttpServletResponse response) {
		// 定义响应model
	...
	public ModelAndView createUser(HttpServletRequest request,
		...
		try {
			...
			if (userService.checkUserByUserName(user.getUserName(), null)) {
				...
				log.info("[EOMS系统]-[创建用户]-[创建失败，当前用户已存在。]");
	 ...
	 * @return
	 */
	public ModelAndView showDispth(HttpServletRequest request,
			HttpServletResponse response) {
		// 定义响应model
	 ...
	 * @return
	 */
	public ModelAndView updateDispth(HttpServletRequest request,
			HttpServletResponse response) {
		// 定义响应model
	 ...
	 * @return
	 */
	public ModelAndView forbiddenUser(HttpServletRequest request,
			HttpServletResponse response) {
	 ...
	 * @return
	 */
	public ModelAndView reOpenUser(HttpServletRequest request,
			HttpServletResponse response) {
		try {
			Map<String, Object> returnMap = new HashMap<String, Object>();
			response.setCharacterEncoding("UTF-8");
	 ...
	 * @return
	 */
	public ModelAndView reSetDispth(HttpServletRequest request,
			HttpServletResponse response) {
		// 定义响应model
	...
	public ModelAndView reSetPwd(HttpServletRequest request,
		...
		try {
			...
			userService.updateUserRnTx(new Integer(id),
					MD5.getMD5Str(passWd.trim()));
			returnMap.put("returnCode", ValTemplate.V_T_VAL_MSG_CODE_0000);
			returnMap.put("description", ValTemplate.V_T_VAL_MSG_RMK_0000);
			result = JSONUtil.toJSonString(returnMap);
	 ...
	 * @return
	 */
	public ModelAndView changePwdDispth(HttpServletRequest request,
			HttpServletResponse response) {
		// 定义响应model
		...
		try {
			...
			if (user != null) {
				...
				if (MD5.getMD5Str(oldPwd).equals(user.getPassWd())) {
					...
					returnMap.put("returnCode",
							ValTemplate.V_T_VAL_MSG_CODE_0011);
					returnMap.put("description",
							ValTemplate.V_T_VAL_MSG_RMK_0011);
				}
			} else {
